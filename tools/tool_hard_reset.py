...
Användning:
    ...
    python tools/tool_hard_reset.py --confirm --no-backup  # Skippa backup
"""
...
def reset_taxonomy():
    """Behåller huvudnoder och beskrivningar, tömmer sub_nodes."""
    if not os.path.exists(TAXONOMY_FILE):
        print(f"  ⏭️  Taxonomi: Finns inte")
        return
    
    with open(TAXONOMY_FILE, 'r', encoding='utf-8') as f:
        taxonomy = json.load(f)
    
    # Behåll description, töm sub_nodes
    cleaned = {}
    for key, value in taxonomy.items():
        if isinstance(value, dict) and "description" in value:
            # Ny struktur: {"description": "...", "sub_nodes": [...]}
            cleaned[key] = {
                "description": value.get("description", ""),
                "sub_nodes": []
            }
        else:
            # Gammal struktur eller tom - sätt till ny struktur
            cleaned[key] = {
                "description": "",
                "sub_nodes": []
            }
    ...
    print(f"  🗑️  Taxonomi: {len(cleaned)} huvudnoder behållna, sub_nodes tömda")
...
""")
    ...
    print("\n✅ RESET KOMPLETT!")
    print("\nNästa steg:")
    print("  1. Starta tjänsterna: python start_services.py")
    print("  2. Transcriber kommer transkribera om alla .m4a")
    print("  3. DocConverter kommer processa alla dokument")
    print("  4. Indexer kommer bygga ny graf och vektorer")
